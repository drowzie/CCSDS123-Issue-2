...
void updateWeightVector(int32_t * weights, int32_t * diffVector, int64_t error, uint16_t x, uint16_t y, uint16_t z, struct arguments * parameters) {
    ...
    for(int i = 0; i < currentPredictionBand; i++) { 
        if((scalingExp+parameters->interbandOffset) > 0) {
            tmp = (signError * diffVector[i]) >> (scalingExp+parameters->interbandOffset);
        } else {
            tmp = (signError * diffVector[i]) << (-1*(scalingExp+parameters->interbandOffset));
        }
        tmp = (tmp + 1) >> 1;
    ...
    if(parameters->mode == FULL) {
        for (int i = 0; i < 3; i++) {
            if((scalingExp+parameters->intrabandExponent) > 0) {
                tmp = (signError * diffVector[parameters->precedingBands + i]) >> (scalingExp+parameters->intrabandExponent);
            } else {
                tmp = (signError * diffVector[parameters->precedingBands + i]) << (-1*(scalingExp+parameters->intrabandExponent));
            }
            tmp = (tmp + 1) >> 1;
