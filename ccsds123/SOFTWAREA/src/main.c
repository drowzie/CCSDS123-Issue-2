...
int main(int argc, char **argv) {
	...
	*/
	...
	uint8_t * compressedImage = malloc(parameters.xSize*parameters.ySize*parameters.zSize*sizeof(uint32_t));
	...
	if(parameters.compressionMode == COMPRESS) {
		sample = malloc(parameters.xSize*parameters.ySize*parameters.zSize*sizeof(uint32_t));
